...
const HouseReg = () => {
    const {setShowHouseReg, axios, getToken, setIsOwner} = useAppContext()
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            const {data} = await axios.post('/api/houses/', {name, contact, address, place }, {headers: {Authorization: `Bearer ${await getToken()}`}})
  ...
  return (
    <div onClick={() => setShowHouseReg(false)} className='fixed top-0 bottom-0 left-0 right-0 z-100 flex items-center justify-center bg-black/70'>
